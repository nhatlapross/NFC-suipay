...
export default function MerchantRegisterPage() {
  ...
  const [apiKeys, setApiKeys] = useState<{publicKey: string, secretKey: string} | null>(null);
  ...
  const [settlementPeriod, setSettlementPeriod] = useState<'daily' | 'weekly' | 'monthly'>('daily');
        ...
        <div className="max-w-2xl w-full">
          <Card className="p-8 border-4 border-black shadow-[8px_8px_0_black]">
            <div className="text-center mb-6">
              ...
              <h1 className="text-3xl font-bold text-black mb-2">MERCHANT REGISTERED!</h1>
              <p className="text-gray-600">Your merchant account has been created successfully</p>
            </div>

            <div className="space-y-4 mb-6">
              <div className="p-4 bg-green-50 border-2 border-green-300 rounded">
                <h3 className="font-bold text-green-800 mb-2">API Keys Generated</h3>
                <div className="space-y-2">
                  <div>
                    <label className="block text-sm font-bold text-green-700">Public Key:</label>
                    <code className="text-xs bg-white p-2 border rounded block break-all">
                      {apiKeys.publicKey}
                    </code>
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-green-700">Secret Key:</label>
                    <code className="text-xs bg-white p-2 border rounded block break-all">
                      {apiKeys.secretKey}
                    </code>
                  </div>
                ...
                <p className="text-xs text-green-600 mt-2">
                  ⚠️ Save these keys securely. You'll need them to access merchant APIs.
                </p>
              </div>
            </div>
  ...
  return (
    <div className="min-h-screen bg-[#f3f4f6] flex items-center justify-center p-4">
      <div className="max-w-2xl w-full">
        <Card className="p-8 border-4 border-black shadow-[8px_8px_0_black]">
          <div className="text-center mb-6">
            <h1 className="text-3xl font-bold text-black mb-2">MERCHANT REGISTRATION</h1>
            <p className="text-gray-600">Complete your merchant account setup</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-bold text-black mb-2">Merchant Name *</label>
                <Input
                  value={merchantName}
            ...
            <div>
              <label className="block text-sm font-bold text-black mb-2">Wallet Address *</label>
              <Input
                value={walletAddress}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-sm font-bold text-black mb-2">Postal Code *</label>
                <Input
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-bold text-black mb-2">Webhook URL</label>
                <Input
                  value={webhookUrl}
              ...
              <div>
                <label className="block text-sm font-bold text-black mb-2">Settlement Period *</label>
                <select
                  value={settlementPeriod}
                  onChange={(e) => setSettlementPeriod(e.target.value as 'daily' | 'weekly' | 'monthly')}

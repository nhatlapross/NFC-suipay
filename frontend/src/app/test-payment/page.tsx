...
import { Loader2, Nfc, CreditCard, CheckCircle, XCircle, ExternalLink } from 'lucide-react';
...
export default function TestPaymentPage() {
  ...
  const [step, setStep] = useState<'idle' | 'nfc-tap' | 'pin-entry' | 'processing' | 'completed'>('idle');
    ...
    try {
      ...
      reader.addEventListener('reading', async ({ message, serialNumber }: any) => {
        ...
        for (const record of message.records) {
          ...
          if (record.recordType === 'text' && data.includes('-') && data.length > 30) {
            console.log('Found UUID in NDEF text record:', data);
            detectedUuid = data;
            break;
          }
        }
        ...
        console.log('🚀 Auto-initiating payment with detected UUID:', detectedUuid);
      ...
      alert(`NFC Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
      setIsScanning(false);
    }
  };
  ...
  const writeUUIDToNFC = async () => {
    ...
    try {
      ...
      alert(`UUID written to NFC card successfully!\n\nUUID: ${cardUuid}\n\nYou can now scan this card to test payments.`);

    } catch (error) {
      console.error('NFC Write Error:', error);
      if (error instanceof Error) {
        if (error.name === 'NotAllowedError') {
          alert('NFC write permission denied. Please allow NFC access and try again.');
        } else if (error.name === 'NetworkError') {
          alert('NFC write failed. Make sure the NFC tag is writable and try again.');
        } else {
  ...
  const initiatePaymentWithNFC = async (detectedUuid: string) => {
    ...
    try {
      console.log('💳 Initiating payment with NFC card UUID:', detectedUuid);
  ...
  const getStepIcon = (currentStep: string) => {
    ...
    switch (currentStep) {
      case 'nfc-tap':
        return <Nfc className={`h-4 w-4 ${step === 'nfc-tap' ? 'text-blue-500' : 'text-gray-400'}`} />;
      case 'pin-entry':
        return <CreditCard className={`h-4 w-4 ${step === 'pin-entry' ? 'text-blue-500' : 'text-gray-400'}`} />;
      case 'processing':
  ...
  return (
    <div className="container mx-auto p-6 max-w-4xl">
      <div className="mb-6">
        <h1 className="text-3xl font-bold mb-2">NFC Payment Test Page</h1>
            ...
            <div className="space-y-3">
              <div>
                  ...
                  {nfcSupported && (
                    <div className="flex gap-1">
                      <Button
                        ...
                        disabled={isScanning || isWriting || step !== 'idle'}
                ...
                <div className="flex items-center gap-2 mt-1">
                  <Badge variant={nfcSupported ? 'default' : 'secondary'} className="text-xs">
                    NFC: {nfcSupported === null ? 'Checking...' : nfcSupported ? 'Supported' : 'Not Supported'}
                  </Badge>
                  {!nfcSupported && nfcSupported !== null && (
                    <span className="text-xs text-gray-500">Use manual UUID entry</span>
                  )}
                </div>
              </div>

              <div className="text-sm text-gray-600 space-y-1">
                <p><strong>Merchant:</strong> {TEST_DATA.merchantId}</p>
                <p><strong>Terminal:</strong> {TEST_DATA.terminalId}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className={`flex items-center gap-3 p-3 rounded ${
              step === 'nfc-tap' ? 'bg-blue-50 border border-blue-200' : ''
            }`}>
              {getStepIcon('nfc-tap')}
            ...
            {/* Step 2: PIN Entry */}
            <div className={`flex items-center gap-3 p-3 rounded ${
              step === 'pin-entry' ? 'bg-blue-50 border border-blue-200' : ''
            }`}>
          ...
          <CardContent>
            {posSession.success ? (
              <div className="space-y-2 text-sm">
                <p><strong>Session ID:</strong> {posSession.sessionId}</p>
                {posSession.displayData && (
                  <>
                    <p><strong>Card Holder:</strong> {posSession.displayData.cardHolder}</p>
                    <p><strong>Amount:</strong> {posSession.displayData.amount}</p>
                    <p><strong>Merchant:</strong> {posSession.displayData.merchantName}</p>
                    <p><strong>Terminal:</strong> {posSession.displayData.terminalName}</p>
                    <p><strong>Auth Required:</strong> {posSession.authRequired?.join(', ')}</p>
                  </>
                )}
              </div>
            ) : (
              <Alert>
                ...
                <AlertDescription>{posSession.error}</AlertDescription>
              </Alert>
            )}
          </CardContent>
        </Card>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-mono text-xs break-all">{paymentResult.transaction.transactionId}</p>
                  </div>
                  <div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-semibold text-green-900">Write UUID to NFC Card</h4>
                  <Button
                    onClick={writeUUIDToNFC}
                    disabled={isWriting || isScanning || !cardUuid.trim()}
                    size="sm"
                ...
                <ol className="text-sm text-green-800 space-y-1">
                  <li>1. Nhập hoặc để UUID mặc định ở trên</li>
                  <li>2. Click "Write UUID" và đặt NFC card gần thiết bị</li>
                  <li>3. UUID sẽ được ghi vào card với NDEF format</li>
                  <li>4. Sau đó có thể scan card để test payment</li>
                </ol>
              </div>

              {/* Read from NFC Card */}
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <h4 className="font-semibold text-blue-900 mb-2">NFC Payment Flow</h4>
                <p className="text-sm text-blue-800 mb-2">
                  Thanh toán tự động sau khi detect NFC:
                </p>
                <ol className="text-sm text-blue-800 space-y-1">
                  <li>1. Click "Scan" button ở trên</li>
                  <li>2. Đặt NFC card gần thiết bị</li>
                  <li>3. Hệ thống tự động tạo payment session</li>
                  <li>4. Màn hình PIN entry sẽ hiện ra ngay lập tức</li>
                  <li>5. Nhập PIN và hoàn thành thanh toán</li>
                </ol>
              </div>
              ...
              {isScanning && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                  <div className="flex items-center gap-2 text-yellow-800">
                    ...
                    <span className="font-medium">NFC Scanning Active</span>
                  </div>
              ...
              {isWriting && (
                <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                  <div className="flex items-center gap-2 text-purple-800">
                    <Loader2 className="h-4 w-4 animate-spin" />
                    <span className="font-medium">NFC Writing Active</span>
                  </div>
                  <p className="text-sm text-purple-700 mt-1">
                    Đang ghi UUID vào NFC card. Đặt card gần thiết bị...
                  </p>
                </div>
              )}
            </div>

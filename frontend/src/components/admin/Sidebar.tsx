...
const Sidebar: React.FC<SidebarProps> = ({ activePage, setActivePage, onCollapseChange }) => {
    ...
    return (
        <div className={`${isCollapsed ? 'w-16' : 'w-64'} bg-black text-white ${isCollapsed ? 'p-2' : 'p-6'} border-r-4 border-black transition-all duration-300 ease-in-out relative`}>
            <div className={`${isCollapsed ? 'mb-6' : 'mb-12'}`}>
                <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'} ${isCollapsed ? 'mb-0' : 'mb-2'}`}>
                    <Image 
                        src="/panda.png" 
                        alt="Admin Logo" 
                ...
                {menuItems.map((item) => {
                    ...
                    return (
                        <button
                            ...
                            className={`w-full flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'} ${isCollapsed ? 'p-2' : 'p-4'} text-left font-bold border-2 border-black transition-all ${
                                isActive
                        ...
                        >
                            <Icon className={`${isCollapsed ? 'w-4 h-4' : 'w-5 h-5'} flex-shrink-0`} />
            ...
            <button
                ...
                className={`absolute ${isCollapsed ? 'top-2 right-2' : 'top-4 right-4'} ${isCollapsed ? 'p-1' : 'p-2'} bg-[#00F0FF] text-black border-2 border-black hover:bg-[#FF005C] hover:text-white transition-all duration-200`}

...
import { getAllCardsAPI, blockCardAPI, unblockCardAPI } from "@/lib/api-client";
...
const WalletManagement: React.FC = () => {
    ...
    useEffect(() => {
        const loadCards = async () => {
            try {
                ...
                console.log('🌐 [WalletManagement] API URL should be: /api/card/admin/all');
                
                if (response?.success && response?.data?.cards) {
                    setCards(response.data.cards);
                    console.log('✅ [WalletManagement] Cards loaded successfully:', response.data.cards);
                } else {
                    console.log('❌ [WalletManagement] No cards found or invalid response:', response);
                    setCards([]);
                }
            } catch (err: any) {
                console.error('💥 [WalletManagement] Error loading cards:', err);
    ...
    const toggleCardStatus = async (cardId: string) => {
        try {
            ...
            const card = cards.find(c => c.id === cardId || c.cardUuid === cardId);
            if (!card) {
            ...
            if (card.blockedAt || !card.isActive) {
                ...
                response = await blockCardAPI(cardId, "Admin blocked via wallet management");
            }

            if (response?.success) {
                console.log('✅ [WalletManagement] Card status updated successfully');
                // Refresh cards
                ...
                console.log('❌ [WalletManagement] Failed to update card status:', response);
                setError("Failed to update card status");
            }
        } catch (err: any) {
            console.error('💥 [WalletManagement] Error toggling card status:', err);
    ...
    const activeCards = cards.filter(card => getCardStatus(card) === "ACTIVE").length;
    const blockedCards = cards.filter(card => getCardStatus(card) === "BLOCKED").length;
    const inactiveCards = cards.filter(card => getCardStatus(card) === "INACTIVE").length;
    const totalBalance = cards.reduce((sum, card) => sum + (card.monthlyLimit - card.monthlySpent), 0);
        ...
        return (
            <div className="flex items-center justify-center min-h-[400px]">
                <div className="text-center">
                    ...
                    <p className="text-lg font-semibold text-black">Loading cards...</p>
                </div>
            </div>
        );
    }
        ...
        <div className="space-y-8 w-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 text-right">
                ...
                <div className="bg-[#FF005C] p-6 border-4 border-black shadow-[6px_6px_0_black]">
                    <div className="text-2xl font-bold text-white">{blockedCards}</div>
                    <div className="font-semibold text-sm text-white">
                        BLOCKED CARDS
                    </div>
                </div>
                <div className="bg-black p-6 border-4 border-black shadow-[6px_6px_0_black]">
                    <div className="text-2xl font-bold text-white">{inactiveCards}</div>
                    <div className="font-semibold text-sm text-white">
                        INACTIVE
                    </div>
                </div>
                <div className="bg-white p-6 border-4 border-black shadow-[6px_6px_0_black]">
                    <div className="text-2xl font-bold text-black">{formatAmount(totalBalance)}</div>
            ...
            <div className="bg-white border-4 border-black p-6 shadow-[6px_6px_0_black]">
                ...
                {cards.length === 0 ? (
                    <div className="text-center py-12">
                        ...
                        <p className="text-lg font-semibold text-gray-600">No cards found</p>
                        <p className="text-sm text-gray-500">Create a card to get started</p>
                    </div>
                ) : (
                    <div className="space-y-4">
                        {cards.map((card) => {
                            const status = getCardStatus(card);
                            const isActionLoading = actionLoading === card.id || actionLoading === card.cardUuid;
                                ...
                                >
                                    ...
                                    <div className="text-right">
                                        <div className="text-xl font-bold text-black">
                                            {formatAmount(card.monthlyLimit - card.monthlySpent)}
                                        </div>
                                    ...
                                    <div
                                        className={`px-3 py-2 border-2 border-black font-bold text-sm flex items-center gap-2 ${getStatusColor(card)}`}
                                    >
                                        {getStatusIcon(card)}
                                        {status}
                                    </div>
                                    ...
                                    <div className="text-sm font-bold text-gray-600">
                                        {formatDate(card.lastUsed || new Date())}
                                    </div>

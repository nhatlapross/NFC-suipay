            ...
            try {
                ...
                if (response?.success && response?.data?.transactions) {
                    const formattedTransactions = response.data.transactions.map((tx: any) => ({
                        id: tx._id || tx.id,
                        type: tx.type === 'topup' ? 'received' : 
                              tx.type === 'payment' ? 'payment' : 
                              tx.type === 'withdraw' ? 'transfer' : 'transfer',
                        amount: tx.amount || 0,
            ...
            <div className="p-6 space-y-6">
                ...
                <div className="flex items-center justify-center min-h-[400px]">
                    <div className="text-center">
                        ...
                        <p className="text-lg font-semibold text-neo-black">Loading transactions...</p>
                    </div>
                </div>
            </div>
        );
                            ...
                            >
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="text-right">
                                        ...
                                        >
                                            {isNegative ? "-" : "+"}
                                            {Math.abs(transaction.amount).toFixed(2)} SUI
                                        </p>

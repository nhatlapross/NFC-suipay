...
export class AdminService {
  ...
  async getTransactions(filter: TransactionFilter, page: number = 1, limit: number = 20) {
    try {
      const query: any = {};
  ...
  async forceRefundTransaction(transactionId: string, reason: string, adminId: string) {
    try {
      const transaction = await TransactionModel.findById(transactionId);
      if (!transaction) {
        throw new Error('Transaction not found');
  ...
  async updateTransactionStatus(transactionId: string, newStatus: string, reason?: string, adminId?: string) {
    try {
      const transaction = await TransactionModel.findById(transactionId);
      if (!transaction) {
        throw new Error('Transaction not found');
      ...
      transaction.status = newStatus as 'pending' | 'processing' | 'completed' | 'failed' | 'cancelled';
  ...
  async getCardById(cardId: string) {
    try {
      const card = await Card.findOne({ $or: [{ _id: cardId }, { cardUuid: cardId }] })
        .populate('userId', 'fullName email')
        .lean();
      ...
      const recentTransactions = await TransactionModel.find({ cardId: card._id })
        .sort({ createdAt: -1 })
        .limit(10)

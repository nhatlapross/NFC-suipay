...
export class MerchantService {
  ...
  private async validateWalletAddress(walletAddress: string): Promise<boolean> {
    try {
  ...
  private calculateNextSettlementDate(period: 'daily' | 'weekly' | 'monthly'): Date {
    const now = new Date();
  ...
  async registerMerchant(data: MerchantRegistrationData): Promise<{
    merchant: IMerchant;
    ...
    try {
      ...
      const merchant = await Merchant.create({
        ...
        apiKeys: {
          ...
          webhookSecret: encryptData(apiKeys.webhookSecret)
        },
  ...
  async getMerchantByPublicKey(publicKey: string): Promise<IMerchant | null> {
    try {
      const merchant = await Merchant.findOne({ 'apiKeys.publicKey': publicKey })
  ...
  async refundPayment(
    ...
    try {
      ...
      if (transaction.merchantId?.toString() !== (merchant._id as any).toString()) {
  ...
  async updateMerchantSettings(merchantId: string, settingsUpdate: any): Promise<any> {
    try {
      const merchant = await Merchant.findOne({ merchantId });
      if (!merchant) {
        throw new Error('Merchant not found');
  ...
  async getWebhooks(merchantId: string): Promise<IWebhook[]> {
    try {
      ...
      const webhooks = await Webhook.find({ merchantId: merchant._id }).select('-secretKey');
      return webhooks;
    } catch (error) {
  ...
  async createWebhook(merchantId: string, webhookData: {
    url: string;
    events: string[];
    description?: string;
  }): Promise<IWebhook> {
  ...
  async updateWebhook(merchantId: string, webhookId: string, updateData: {
    url?: string;
    events?: string[];
    description?: string;
    isActive?: boolean;
    ...
    try {
      ...
      if (webhook.merchantId.toString() !== (merchant._id as any).toString()) {
      ...
      if (updateData.description !== undefined) webhook.description = updateData.description;
      if (updateData.isActive !== undefined) webhook.isActive = updateData.isActive;
  ...
  async createApiKey(merchantId: string, keyData: {
    name: string;
    permissions?: string[];
    rateLimit?: {
  ...
  async deleteApiKey(merchantId: string, keyId: string): Promise<void> {
    try {
      ...
      if (apiKey.merchantId.toString() !== (merchant._id as any).toString()) {
  ...
  async validateApiKey(publicKey: string, secretKey: string): Promise<{
    isValid: boolean;
    merchant?: IMerchant;
    apiKey?: IApiKey;
    error?: string;
    ...
    try {
      ...
      if (merchant) {
        ...
        logger.info('Comparing keys directly', {
        ...
        if (storedSecretKey === secretKey) {
          logger.info('API key validation successful for merchant:', merchant.merchantId);
          return {
            isValid: true,
            merchant,
            apiKey: {
        ...
        logger.info('No merchant found for public key:', publicKey.substring(0, 10) + '...');
      }

      logger.info('Checking ApiKey collection');
      ...
      logger.error('Error validating API key - detailed:', {
  ...
  async updateMerchantStatus(merchantId: string, status: string, reason?: string): Promise<IMerchant> {
    try {
      const merchant = await Merchant.findOne({ merchantId });
      if (!merchant) {
        throw new Error('Merchant not found');
  ...
  async updateMerchantLimits(merchantId: string, limits: {
    dailyLimit?: number;
    monthlyLimit?: number;
    commission?: number;
  }): Promise<IMerchant> {

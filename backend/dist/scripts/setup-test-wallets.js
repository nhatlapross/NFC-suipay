...
async function setupTestWallets() {
    try {
        ...
        const encryptedUserKey = typeof userPrivateKey === 'string'
            ...
            : (0, encryption_service_1.encryptPrivateKey)(privateKeyBase64);
        // Update test user with wallet
        const testUser = await User_model_1.User.findOne({ email: 'user@example.com' });
        if (testUser) {
        ...
        const merchantKeypair = new ed25519_1.Ed25519Keypair();
        ...
        const testMerchant = await Merchant_model_1.Merchant.findOne({ merchantId: 'MERCHANT_001' });
        if (testMerchant) {
...
// Run the setup
setupTestWallets();
//# sourceMappingURL=setup-test-wallets.js.map
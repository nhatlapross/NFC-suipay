...
async function testPaymentFlow() {
    ...
    try {
        ...
        try {
            const validationResponse = await axios_1.default.post(`${API_URL}/api/payment/nfc-validate`, {
        ...
        const paymentResponse = await axios_1.default.post(`${API_URL}/api/payment/process-async`, testData, {
            headers: {
        ...
        const statusResponse = await axios_1.default.get(`${API_URL}/api/payment/transactions/${transactionId}`, {
            headers: {
                'Authorization': `Bearer ${TOKEN}`,
            },
        });
        ...
        const historyResponse = await axios_1.default.get(`${API_URL}/api/payment/transactions`, {
            headers: {
                'Authorization': `Bearer ${TOKEN}`,
            },
            params: {
        ...
        console.log('Recent transactions:', historyResponse.data.transactions.slice(0, 3).map((tx) => ({
            id: tx._id,
            amount: tx.amount,
            status: tx.status,
        ...
        const statsResponse = await axios_1.default.get(`${API_URL}/api/payment/stats`, {
            headers: {
                'Authorization': `Bearer ${TOKEN}`,
            },
            params: {
...
function _setupWebSocketMonitoring(transactionId) {

...
class UserController {
    ...
    async updateProfile(req, res, next) {
        try {
            ...
            const saved = await User_model_1.User.findByIdAndUpdate(req.user._id, update, { new: true });
            if (!saved) {
    ...
    async updateSettings(req, res, next) {
        try {
            ...
            const user = await User_model_1.User.findById(req.user._id).select('+twoFactorSecret');
            if (!user)
                return res.status(404).json({ success: false, error: 'User not found' });
            if (typeof twoFactorAuth === 'boolean')
                user.twoFactorEnabled = twoFactorAuth;
            await user.save();
            return res.json({
                success: true,
                settings: {
    ...
    async setPin(req, res, next) {
        try {
            ...
            const user = await User_model_1.User.findById(req.user._id).select('+pinHash');
            if (!user) {
                return res.status(404).json({ success: false, error: 'User not found' });
            }
